...
def transfer_function(ds, **kwargs):
    qqq = "GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA airflow TO postgres;"
    create_query = "create table airflow.abc(city_name varchar(50), city_code varchar(20));"
    # insert_query = "insert into airflow.abc(city_name, city_code) values('Hyderabad', 'hyd');"
    query = "select * from airflow.abc"
    # create_table = "create table airflow..xyz(city_name varchar(50), city_code varchar(20));"
    ...
    if records:
        ...
        execute_values(destination_cursor, "insert into airflow.xyz values %s", records)
...
t3 = PythonOperator(task_id='table', python_callable=list_tables, provide_context=True, dag=dag)
t2>>t1>>t3
